...
COF_IASINT::COF_IASINT(long zoneId, long timeId) {
	...
	for (long i = 0, k = 0; i < 3; ++i) {
		for (long j = 0; j < STORE.surfaceTypeCount_; ++j) {
...
COF_IASINT::COF_IASINT(long monthCount, long latCount, long surfaceTypeCount) {
...
void COF_IASINT::init(long monthCount, long latCount, long surfaceTypeCount) {
	...
	const long count = surfaceTypeCount * latCount * monthCount;
...
protected:
	COF_IASINT read(istream& is) {
		...
		readLongLongBE(is, lats, latCount);
